...
def afterContentCreated(obj, event):
    if obj.package_content:
        ...
        zipTree = OOBTree()
        zipTree[str(content_hash)] = OOBTree()
        extract_package_content(zipTree[content_hash], obj.package_content)
...
def afterContentModified(obj, event):
    if obj.package_content:
        ...
        zipTree.clear()
        zipTree[new_hash] = OOBTree()
        extract_package_content(zipTree[new_hash], obj.package_content)
        setattr(obj, 'zipTree', zipTree)
